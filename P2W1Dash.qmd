---
title: "Democracy Across the Globe"
format:
  dashboard:
    theme: solar
    fill: true
...
dyn_plot <- ggplot(vis2, aes(log(pop_prop),spatial_democracy,color = region))+
  ...
  geom_density_2d()+
  labs(
    x = "Population Adjusted Representation (People/Representative)",
    y = "Spatial Democracy Score",
    title = "Spatial Democracy vs. Population Adjusted Representation"
  )
...
shiny_df <- shiny_df %>%
  mutate(
    rep_type = case_when(
      pop_prop_log <= 8 ~ "High",
      pop_prop_log > 8 & pop_prop_log <=11 ~ "Moderate",
      pop_prop_log >11 & pop_prop_log <=17 ~ "Low"
    )
  )
...
The **United States** is a large federal democracy that shows patterns relating to global trends. This makes it a useful representation of democracy trends in the modern day first world. The map below shows the democracy index of each US state, ranging from less than -1.5 to greater than 1.
